...
algorithm that was recently described by \cite{pannekoek:2012}. There are are
...
on the order of $10^5-10^6$ variables and $10^4-10^5$ restrictions. Convergence
is usually reached within seconds.
\end{abstract}
...
The core routines are implemented as \code{C} routines (following the
...
standards to me. Any remaining bugs are of course of my doing. 
