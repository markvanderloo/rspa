...
that it obeys the system of (in)equations $\bs{Ax}\leq \bs{b}$.  The package
implements the the successive projection algorithm of \cite{pannekoek:2012}.
The are are several ways to define what ``minimal'' means here, and the package
allows for fairly large systems of equations.  Thus far it has been tested on
systems with on the order of $10^5-10^6$ variables and $10^4-10^5$
restrictions. Convergence is usually reached within seconds.
\end{abstract}
